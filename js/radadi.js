...
function updateList(){
  ...
  function classheader(classname, dist, controls, newpage){
    ...
    var cont = (newpage && pagecount>1) ? ' <i>(continued)</i>' : '';
    return '<div class="classheader"><h2>'+classname+cont+' <span class="coursedata">'+dist+'km / '+controls+' controls</span></h2></div>';
  }
  ...
  var subtitle = (clcfg['type'] == 'resultlist' ? 'Preliminary Results' : 'Start List') + ' - '+evcfg['stagename'];
  ...
  $.each(newData['list'],function(no,line){
    ...
    if(clcfg['type'] == 'resultlist'){
      stno_pl = '<td class="col_place">'+line['Place']+(line['Place'] != '' ? '.' : '');
